...
class LearnersEnsembleHyperParameterized(LearnersEnsemble):
    def evaluate_iterator(self, iterator):
        """
        Evaluate a ensemble of learners on iterator.
        ...
        with torch.no_grad():
            for (x, y, _) in iterator:
                ...
                global_loss += learner.criterion(y_pred, y).sum().detach().item()
                global_metric += learner.metric(y_pred, y).detach().item()
