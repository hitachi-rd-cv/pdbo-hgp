...
def get_config():
    ...
    conf = dict(name_hyperparam=NamesHyperParam.LEARNERS_WEIGHTS_AND_SINGLE_SOFTMAX_LOGITS_WEIGHTS, hyper_learning_rate=hyper_learning_rate, model=model, n_steps=n_steps, lr=10.)
    names_option.append(", ".join([f"{k}={v}" for k, v in conf.items()]))
    configs_overwrite.append(get_config_overwrite(**conf))
